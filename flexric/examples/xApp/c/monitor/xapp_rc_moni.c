...
int main(int argc, char* argv[])
{
  ...
  // e2_nodes_xapp_api() retrieves a list of all interactable E2 nodes for the xApp
  // Assign these usable nodes to an ADT array-like container for storage and recall
  e2_node_arr_xapp_t nodes = e2_nodes_xapp_api();
  // defer: do not execute the code until the current scope is left
  // a "scheduled clean"
  defer({ free_e2_node_arr_xapp(&nodes); });

  assert(nodes.len > 0); // make sure there are E2 nodes
  ...
  for (int i = 0; i < nodes.len; i++) {
    ...
    if (n->rf[idx].defn.rc.report != NULL) {
      ...
      // hndl[] is a struct containing each handle that represents a subscription to an E2 node
