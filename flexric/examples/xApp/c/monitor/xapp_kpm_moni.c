...
void match_meas_name_type(meas_type_t meas_type, meas_record_lst_t meas_record)
{
  ...
  printf("MEAS NAME: %.*s, value_type index: %d\n", (int)meas_type.name.len, meas_type.name.buf, meas_record.value);

  // Get the value of the Measurement
  printf("INCOMING MEASUREMENT: %.*s\n", (int)meas_type.name.len, meas_type.name.buf);

  if (meas_record.value >= END_MEAS_VALUE || get_meas_value[meas_record.value] == NULL) {
    printf("Unsupported measurement value type: %d\n", meas_record.value);
    return;
  }
